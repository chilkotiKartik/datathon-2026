...
$env:PORT = "5000"
$bp = Start-Process -FilePath "pwsh" -WindowStyle Hidden -ArgumentList "-NoLogo","-NoProfile","-Command","& { `$env:PORT='5000'; Set-Location '$rootDir\backend'; npx tsx server/index.ts }"

Start-Sleep -Seconds 8

Write-Host "[3/3] Starting Frontend (port 3000)..."
$fp = Start-Process -FilePath "pwsh" -WindowStyle Hidden -ArgumentList "-NoLogo","-NoProfile","-Command","& { Set-Location '$rootDir\frontend'; npx vite --port 3000 --host 0.0.0.0 }"
...
try {
  ...
  Write-Host "✅ Health: status=$($c.status), crimes=$($c.crimeCount)"
} catch { Write-Host "❌ Backend not ready yet (seeding in progress)" }
...
} catch { Write-Host "❌ Frontend not ready yet" }
